...
def _chart_entry(dataset_name: str, iteration: int, file_name: str, title: str, description: str) -> Dict[str, str]:
    return {
        "id": file_name.replace(".png", ""),
        "title": title,
        "description": description,
        "file": file_name,
        "url": f"/api/chart-image/{dataset_name}/{iteration}/{file_name}",
    }


def _iteration_metric_history(dataset_name: str, upto_iteration: int) -> List[Dict[str, float]]:
    out_dir = os.path.join(OUTPUT_ROOT, dataset_name)
    rows: List[Dict[str, float]] = []
    for current in range(1, max(int(upto_iteration), 1) + 1):
        file_path = os.path.join(out_dir, f"iteration_{current}_recs.json")
        ...
        try:
            ...
            continue
        metrics = payload.get("system_metrics") or {}
        rows.append(
            {
                "iteration": float(current),
                "coverage": float(metrics.get("coverage_top_rules", 0.0) or 0.0),
                "uplift": float(metrics.get("estimated_uplift_score", 0.0) or 0.0),
                "rule_count": float(metrics.get("n_rules_blended", 0.0) or 0.0),
                "drift_js": float((payload.get("drift") or {}).get("js", 0.0) or 0.0),
            }
        )
    return rows


def generate_iteration_charts(
    dataset_name: str,
    iteration: int,
    ...
    if top_items:
        ...
        axis.set_title("Top Items by Purchase Count")
        ...
        chart_manifest.append(
            _chart_entry(
                ...
                file_name=file_name,
                title="Top Items",
                description="Most purchased items in the selected iteration.",
            )
        )

    if not blended_rules.empty:
        sample = blended_rules.head(140).copy()
        bubble_sizes = 80 + (sample["support"].fillna(0.0).to_numpy() * 2600)
        fig, axis = plt.subplots(figsize=(9.5, 4.8))
        scatter = axis.scatter(
            sample["confidence"].to_numpy(),
            sample["lift"].to_numpy(),
            s=bubble_sizes,
            c=sample["blend_score"].fillna(0.0).to_numpy(),
            cmap="YlOrRd",
            alpha=0.75,
            edgecolors="#6b1f1f",
            linewidths=0.4,
        )
        axis.set_title("Rule Quality Map")
        ...
        chart_manifest.append(
            _chart_entry(
                ...
                file_name=file_name,
                title="Rule Quality",
                description="Confidence vs lift with bubble size based on support.",
            )
        )

    segment_counts = (
        transactions_df["segment"]
        .fillna("unknown")
        .astype(str)
        .str.lower()
        .value_counts()
        .reindex(["morning", "lunch", "dinner"], fill_value=0)
    )
    if int(segment_counts.sum()) > 0:
        ...
        axis.set_title("Transaction Mix by Segment")
        ...
        chart_manifest.append(
            _chart_entry(
                ...
                file_name=file_name,
                title="Segment Mix",
                description="Distribution of morning, lunch, and dinner transactions.",
            )
        )

    if not menu_rank.empty:
        top_rank = menu_rank.head(10).copy()
        fig, axis = plt.subplots(figsize=(9.5, 4.8))
        axis.plot(
            top_rank["item"].astype(str).tolist(),
            top_rank["rank_score"].fillna(0.0).tolist(),
            marker="o",
            color="#b91c1c",
            linewidth=2.2,
        )
        axis.set_title("Homepage Rank Score by Item")
        ...
        chart_manifest.append(
            _chart_entry(
                ...
                iteration=iteration,
                file_name=file_name,
                title="Homepage Ranking Curve",
                description="Comparison of rank scores for top homepage candidates.",
            )
        )

    history_rows = _iteration_metric_history(dataset_name=dataset_name, upto_iteration=iteration)
    if history_rows:
        ...
        axes[0].set_title("Coverage and Uplift Trend")
        ...
        axes[1].set_title("Rule Count and Drift")
        ...
        chart_manifest.append(
            _chart_entry(
                ...
                iteration=iteration,
                file_name=file_name,
                title="Iteration Trend",
                description="Coverage, uplift, rule-count, and drift comparison across iterations.",
            )
        )
...
def run_dataset_iterations(dataset_name: str, max_iteration: int = 3) -> List[Dict[str, object]]:
    ...
    for iteration in range(1, max_iteration + 1):
        ...
        chart_manifest = generate_iteration_charts(
            dataset_name=dataset_name,
            iteration=iteration,
            transactions_df=df,
            baskets=baskets,
        ...
        rec_json["charts"] = chart_manifest
        with open(
            os.path.join(out_dir, f"iteration_{iteration}_recs.json"), "w", encoding="utf-8"
        ) as handle:
            json.dump(rec_json, handle, indent=2, ensure_ascii=False)
