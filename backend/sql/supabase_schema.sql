...
-- Safe to rerun: all objects are created/updated idempotently.
...
-- Backfill columns for existing deployments that were created before this script version.
alter table public.cb_transactions add column if not exists updated_at timestamptz not null default now();
alter table public.cb_transactions alter column batch_no set default 1;
update public.cb_transactions set batch_no = 1 where batch_no is null;

create unique index if not exists cb_transactions_dataset_txid_uidx
  on public.cb_transactions(dataset_key, tx_id);

create index if not exists cb_transactions_dataset_idx
  on public.cb_transactions(dataset_key);
...
create index if not exists cb_item_margins_dataset_idx
  on public.cb_item_margins(dataset_key);
...
drop trigger if exists cb_transactions_set_updated_at on public.cb_transactions;
create trigger cb_transactions_set_updated_at
before update on public.cb_transactions
for each row execute function public.cb_set_updated_at();
